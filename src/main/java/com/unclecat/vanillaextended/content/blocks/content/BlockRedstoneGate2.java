...
public class BlockRedstoneGate2 extends BlockBase implements ITileEntityProvider
{
	...
	/* Will be saved in NBT but not in meta when unloaded on disk */ public static final PropertyDirection OUTPUT_SIDE = createProp("output_side", EnumFacing.Plane.HORIZONTAL, EnumFacing.NORTH);
    protected static final AxisAlignedBB THIS_AABB = createAABB(0, 0, 0, 16, 2, 16);
    
	
	
	public BlockRedstoneGate2(String name, boolean isPoweredType) 
	{
	...
	{
		return new BlockRedstoneGate2.ThisTileEntity();
	}
	
	@Override
	protected BlockStateContainer createBlockState()
    {
		return new BlockStateContainer(this, new IProperty[] {NORTH_INVERTED, WEST_INVERTED, SOUTH_INVERTED, EAST_INVERTED, OUTPUT_SIDE});
    }
	
	@Override
	public int getMetaFromState(IBlockState state)
	{
		return (state.getValue(NORTH_INVERTED) ? 1 : 0) +
				(state.getValue(WEST_INVERTED) ? 2 : 0) +
				(state.getValue(SOUTH_INVERTED) ? 4 : 0) +
				(state.getValue(EAST_INVERTED) ? 8 : 0);
	}
	@Override
	public IBlockState getStateFromMeta(int meta)
	{
		return blockState.getBaseState()
				.withProperty(NORTH_INVERTED, (meta & 1) > 0)
				.withProperty(WEST_INVERTED, (meta & 2) > 0)
				.withProperty(SOUTH_INVERTED, (meta & 4) > 0)
				.withProperty(EAST_INVERTED, (meta & 8) > 0);
	}
	
	public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state)
    {
//		worldIn.setBlockState(pos, state.withProperty(OUTPUT_SIDE, ((BlockRedstoneGate.ThisTileEntity)worldIn.getTileEntity(pos)).outputSide));
//		worldIn.markChunkDirty(pos, null);
//		((BlockRedstoneGate.ThisTileEntity)worldIn.getTileEntity(pos)).markDirty();
    }
	
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos)
    {
        return state;//.withProperty(OUTPUT_SIDE, ((ThisTileEntity)worldIn.getTileEntity(pos)).outputSide);
    }
    ...
    {
		worldIn.setBlockState(pos, state.withProperty(OUTPUT_SIDE, placer.getHorizontalFacing()));
		((BlockRedstoneGate2.ThisTileEntity)worldIn.getTileEntity(pos)).outputSide = (placer.getHorizontalFacing());
		((BlockRedstoneGate2.ThisTileEntity)worldIn.getTileEntity(pos)).markDirty();
...
//    	System.out.println("GGGGG");
        ...
        {
        	 return blockState.getValue(OUTPUT_SIDE).getOpposite() == side ? 15 : 0;
        }
        else
        {
           return 0;
...
//    	if (worldIn.isRemote) return;
    	...
    	EnumFacing outputSide = state.getValue(OUTPUT_SIDE);
    	
//    	if (fromPos.offset(outputSide).equals(pos)) return;
    	
    	if (outputSide != EnumFacing.NORTH && state.getValue(NORTH_INVERTED) != calculateInputStrength(worldIn, EnumFacing.NORTH, pos, state) > 0)
    	{
    		preOutput = true;
    	} 
    	else if (outputSide != EnumFacing.WEST && state.getValue(WEST_INVERTED) != calculateInputStrength(worldIn, EnumFacing.WEST, pos, state) > 0)
    	{
    		preOutput = true;
    	} 
    	else if (outputSide != EnumFacing.SOUTH && state.getValue(SOUTH_INVERTED) != calculateInputStrength(worldIn, EnumFacing.SOUTH, pos, state) > 0)
    	{
    		preOutput = true;
    	} 
    	else if (outputSide != EnumFacing.EAST && state.getValue(EAST_INVERTED) != calculateInputStrength(worldIn, EnumFacing.EAST, pos, state) > 0)
    	{
    		preOutput = true;
    	} 
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        worldIn.setBlockState(pos, worldIn.getBlockState(pos).withProperty(OUTPUT_SIDE, ((BlockRedstoneGate2.ThisTileEntity)worldIn.getTileEntity(pos)).getOutputSide()));
        BlockRedstoneGate2.ThisTileEntity te = new BlockRedstoneGate2.ThisTileEntity((BlockRedstoneGate2.ThisTileEntity)worldIn.getTileEntity(pos));
        te.setOutputSide(state.getValue(OUTPUT_SIDE));
...
//    		System.out.println("EAST_INVERTED");
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        if (!worldIn.isRemote && facing == EnumFacing.UP)
        {
        	...
        	{
        		worldIn.playSound(null, pos, SoundEvent.REGISTRY.getObject(new ResourceLocation("block.lever.click")), SoundCategory.BLOCKS, 0.3f, state.getValue(switched) ? 0.5f : 0.6f);
        		state = state.withProperty(switched, !state.getValue(switched));
        		worldIn.setBlockState(pos, state);
        		neighborChanged(state, worldIn, pos, null, pos.offset(state.getValue(OUTPUT_SIDE)));
        	}	
        }
        return true;
    }
	...
	public class ThisTileEntity extends TileEntity
	{
		...
		public ThisTileEntity(ThisTileEntity that)
		{
			this.outputSide = that.outputSide;
			//markDirty();
		}
		...
		public void onLoad()
		{
			world.setBlockState(pos, world.getBlockState(pos).withProperty(OUTPUT_SIDE, outputSide));
		}
		...
		public void readFromNBT(NBTTagCompound compound)
	    {
	        ...
	        world.setBlockState(pos, world.getBlockState(pos).withProperty(OUTPUT_SIDE, EnumFacing.HORIZONTALS[compound.getInteger("outputSide")]));
...
//	        compound.setInteger("outputSide", outputSide.getHorizontalIndex());
	        compound.setInteger("outputSide", this.world.getBlockState(pos).getValue(OUTPUT_SIDE).getHorizontalIndex());
